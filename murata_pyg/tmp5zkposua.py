...
class Propagate_faacf1(NamedTuple):
    x: OptPairTensor
    edge_weight: OptTensor



class Collect_faacf1(NamedTuple):
    x_j: torch.Tensor
...
class GravNetConvJittable_faacf1(GravNetConv):
    ...
    def __collect__(self, edge_def, size, kwargs):
        # type: (Tensor, List[Optional[int]], Propagate_faacf1) -> Collect_faacf1
        pass

    @torch.jit._overload_method
    def __collect__(self, edge_def, size, kwargs):
        # type: (SparseTensor, List[Optional[int]], Propagate_faacf1) -> Collect_faacf1
        pass
    ...
    def __collect__(self, edge_def, size, kwargs):
        ...
        return Collect_faacf1(x_j=x_j, edge_weight=edge_weight, index=index, dim_size=dim_size)
    ...
    def propagate(self, edge_index, x, edge_weight, size=None):
        ...
        in_kwargs = Propagate_faacf1(x=x, edge_weight=edge_weight)

        

        kwargs = self.__collect__(edge_index, the_size, in_kwargs)
        out = self.message(x_j=kwargs.x_j, edge_weight=kwargs.edge_weight)
        out = self.aggregate(out, index=kwargs.index, dim_size=kwargs.dim_size)
