...
class Event:
    ...
    # ak_pred: beta, x, y
    @classmethod
    def from_awk(cls, ak_feat, ak_label, ak_pred, ak_energy, ak_trans_pred, i):
        feat = ak_feat[i]
        label = ak_label[i]
        pred = ak_pred[i]
        trans_pred = ak_trans_pred[i]
        ...
        print(ak.to_numpy(trans_pred).shape)
        inst.trans_clusid = trans_pred[:,0]
        inst.trans_truth_four_vec = trans_pred[:,1:5]
        inst.trans_pred_four_vec = trans_pred[:,5:9]
    ...
    def __getitem__(self, where):
        ...
        new.trans_clusid = self.trans_clusid[where]
        new.trans_truth_four_vec = self.trans_truth_four_vec[where]
        new.trans_pred_four_vec = self.trans_pred_four_vec[where]
...
def plot_event_trans_pred(e: Event, dx=0, dy=1):

    pdata = []
    colorwheel = ColorWheel()
    beta_th = 0.2
    clusters = e.trans_clusid
        ...
        for tr in (1, 0):
            ...
            pdata.append(go.Scatter(
                ...
                text=[
                    ...
                    f'<br>pdgid={int(pdgid)}'
                    f'<br>mcen={mcen:.3f}<br>Epred={pred_e_cond:.3f}<br>Epred_calo={pred_e_calo:.3f}'
                    f'<br>four momentum (truth)=({trans_truth_four_vec[0]:.3f}, {trans_truth_four_vec[1]:.3f}, {trans_truth_four_vec[2]:.3f}, {trans_truth_four_vec[3]:.3f})'
                    f'<br>four momentum (pred)=({trans_pred_four_vec[0]:.3f}, {trans_pred_four_vec[1]:.3f}, {trans_pred_four_vec[2]:.3f}, {trans_pred_four_vec[3]:.3f})'
                    for beta, x, y, z, energy, track, pdgid, mcen, pred_e_cond, pred_e_calo, trans_truth_four_vec, trans_pred_four_vec
                    in zip(e.beta[sel2], e.x[sel2], e.y[sel2], e.z[sel2], e.energy[sel2], e.track[sel2], e.pdgid[sel2], e.mcen[sel2], e.pred_e_cond[sel2], e.pred_e_calo[sel2], e.trans_truth_four_vec[sel2], e.trans_pred_four_vec[sel2])
                ],
                hovertemplate=(
                    f'x=%{{x:0.2f}}<br>y=%{{y:0.2f}}'
                    f'<br>%{{text}}'                
...
def main():
    ...
    ak_feat, ak_label, ak_pred, ak_energy, ak_pandora, _, _ = tools.load_awkward.load_awkward2(awkfile)
    ak_trans_pred = tools.load_awkward.load_awkward2_transformerPrediction(awkfile)
    ...
    for i in range(nstart,nend):
        e = Event.from_awk(ak_feat, ak_label, ak_pred, ak_energy, ak_trans_pred, i)
        ...
        for ii in range(0,math.ceil(virtual_space_coordinate_dim/2)):
            ...
            single_pdata_to_file_pred(outhtml, plot_event_trans_pred(e, dx=dx, dy=dy), title=title, include_plotlyjs=True, mode=mode)
