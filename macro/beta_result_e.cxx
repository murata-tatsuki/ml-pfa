...
void beta_result_e(){ 
    ...
    TGraph *eff_vs_pur_50[nParticle];
    for(int ip=0;ip<nParticle;ip++){
        ...
        eff_vs_pur_50[ip] = new TGraph();
        eff_vs_pur_50[ip]->SetTitle(Form("%s",particleNames[ip].c_str()));
        eff_vs_pur_50[ip]->GetXaxis()->SetTitle("efficiency");
        eff_vs_pur_50[ip]->GetYaxis()->SetTitle("puriy");
    ...
    for(int irawfile=0; irawfile<rawfilenum; irawfile++){
        ...
        for(int ip=0;ip<nParticle;ip++){
            efficiency_scan[ip][irawfile] = efficiency[ip]->GetMean();
            purity_scan[ip][irawfile] = purity[ip]->GetMean();

            eff_vs_pur[ip]->SetPoint(irawfile, efficiency_scan[ip][irawfile], purity_scan[ip][irawfile]);
            if(irawfile==50) eff_vs_pur_50[ip]->SetPoint(irawfile, efficiency_scan[ip][irawfile], purity_scan[ip][irawfile]);
        }
