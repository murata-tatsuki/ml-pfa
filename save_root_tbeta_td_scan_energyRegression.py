...
def save_root(datapath, ckpt, outfileDir, nstart=0, nend=-1, timingCut=False, input_dim=5, output_dim=3, pandora=False, energyRegression=False, momentum=False, momentumAmp=False):
    debug = False
    ...
    for tbeta_now in range(1,2):
        # for td_now in range(1,10):
        for td_now in range(6,7):
            tbeta = tbeta_now / 10
            td = td_now / 10
...
def main():
    if (len(sys.argv) != 13):
        print("Usage: save_root.py datapath ckpt outfile nstart nend timingCut input_dim output_dim pandora energyRegression momentum momentumAmp")
        return
    
    save_root(sys.argv[1],sys.argv[2],sys.argv[3],nstart=int(sys.argv[4]),nend=int(sys.argv[5]),timingCut=strtobool(sys.argv[6]),input_dim=int(sys.argv[7]), output_dim=int(sys.argv[8]), pandora=strtobool(sys.argv[9]), energyRegression=strtobool(sys.argv[10]), momentum=strtobool(sys.argv[11]), momentumAmp=strtobool(sys.argv[12]))
