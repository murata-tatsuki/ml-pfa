...
def save_root(datapath, ckpt_gnn, ckpt_clustering, outfile, nstart=0, nend=-1, timingCut=False, input_dim=5, output_dim=3, args={}):
    ...
    model_clustering = get_clustering_model(ckpt_clustering, jit=False, input_dim=input_dim,output_dim=output_dim, lcr_block=args.lcr_block, ddp=args.ddp, pid=args.pid)
    ...
    yielder = TestYielderWithMLClustering_trackQuery(model=model_gnn, model_clustering=model_clustering, energyRegression=energyRegression, energyRegressionCluster=energyRegressionCluster, classification=args.classification, pid=args.pid, dataset=dataset, device=device, pandora=pandora)
