        ...
        for td in td_list:
            ...
            for i, (event, prediction, clustering, matches, condensation_points) in enumerate(yielder.iter_matches(tbeta=tbeta, td=td, nmax=nmax, pandora=pandora, energyRegression=energyRegression, energyRegressionCluster=energyRegressionCluster)):
                ...
                for id in all_truth_ids:
                    ...
                    predicted_beta = np.array([])
                    predicted_energy = np.array([])
                    predicted_energy_cluster = np.array([])
                    predicted_energy_cluster = np.array([])
                    ...
                    if (id in matches12.keys()):
                        ...
                        for rid in reco_match:
                            ...
                            # predicted_beta = np.concatenate([predicted_beta, prediction.pred_betas[pattern_cluster]])
                            # predicted_energy = np.concatenate([predicted_energy, prediction.pred_tracker_energy[pattern_cluster]])
                            # predicted_energy_cluster = np.concatenate([predicted_energy_cluster, prediction.pred_cluster_energy[pattern_cluster]])
                            # matched_tracks = np.concatenate([matched_tracks, match_track[pattern_cluster]])
                            ...
                            predicted_beta = prediction.pred_betas[pattern_cluster]
                            predicted_beta = -np.sort(-predicted_beta)
                            dict_beta_pattern[rid] = {"beta":predicted_beta[0], "pattern":pattern_cluster}

                        sorted_dict_beta_pattern = sorted(dict_beta_pattern, key=lambda x:x['beta'])
                        ...
                        if not pandora:
                            ...
                            # print(pattern_cluster, )
                            # match_track = match_track[pattern_cluster]
                            # cond_tracknesses = matched_tracks[np.argsort(-predicted_beta)]
                            # cond_trackness = cond_tracknesses[0]
                            cond_trackness = 0
                            predicted_beta = -np.sort(-predicted_beta)
                            # print(predicted_beta[0], cond_trackness)
