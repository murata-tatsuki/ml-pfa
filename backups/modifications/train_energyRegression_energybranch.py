...
def main():
    ...
    parser.add_argument('-i-tune', '--inputdir-tune', type=str, help='Specify input directory for training (option)')
    ...
    parser.add_argument('--energy-branch', action='store_true', help='Add absoute momentum to GNN input')
    ...
    dataset = ILCDataset(args.inputdir,timingCut=args.timing_cut,thetaphi=args.thetaphi,test_mode=True,momentum=args.momentum,momentumAmp=args.momentum_amp)
    if (args.inputdir_tune and args.inputdir_validate_tune is not None):
        dataset_tune = ILCDataset(args.inputdir_tune,timingCut=args.timing_cut,thetaphi=args.thetaphi,test_mode=True,momentum=args.momentum,momentumAmp=args.momentum_amp)
    ...
    if (args.no_split):
        ...
        test_dataset = ILCDataset(args.inputdir_validate,timingCut=args.timing_cut,thetaphi=args.thetaphi,test_mode=True,momentum=args.momentum,momentumAmp=args.momentum_amp)
        if (args.inputdir_tune and args.inputdir_validate_tune is not None):
            train_dataset_tune = dataset_tune
            test_dataset_tune = ILCDataset(args.inputdir_validate_tune,timingCut=args.timing_cut,thetaphi=args.thetaphi,test_mode=True,momentum=args.momentum,momentumAmp=args.momentum_amp)
    else:
