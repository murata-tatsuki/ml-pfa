...
def main():
    ...
    # print(torch.cuda.device_count())  # 認識されているGPU数
    # print(torch.cuda.get_device_name(0))  # cuda:0のGPU名
    # print(torch.cuda.get_device_name(1))  # cuda:1のGPU名（エラーが発生しないか確認）
    # print(torch.cuda.is_available())  # CUDAが利用可能か
    # print(torch.cuda.current_device())  # 現在のデフォルトデバイス
    ...
    def loss_fn(out, data, i_epoch=None, return_components=False, use_charge_track_likeness=False):
        ...
        if args.energy_regression:
            ...
            LV, Lbeta, LE, LE_charge, out_oc = oc.calc_LV_Lbeta_(
                pred_betas,
                pred_cluster_space_coords,
                pred_charge_track_likeness,
                data.y[:,0].long(),
