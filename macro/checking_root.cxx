...
///////////////////
// usage 
// change the fileName to the root file to analyze
// "root checking_root.cxx"
///////////////////
// displaying effiency, purity, (energy regression) of the root files


// conditions
const bool saving_canvas = false;
const string pdfDirectory = "../pic/energy_regression_1to1";
...
const bool pandora = false;                              // pandoraPFA result


const bool ECluster = true;


const double beta_threshold = 0;          // ignore clusters that have lower condensation point beta than this threshold  0<beta<1


const string fileName = "../output_old/regression/tc_ntau_10GeV_10_5D/no_E_regression/2025_03_25_141559_outputD5_epoch59_ReduceLROnPlateau.root";
...
void checking_root(){ 
    ...
    string picDirectory = pdfDirectory;
    string fileName_ = "";
    
    fileName_ = fileName;

    filein[0] = new TFile(Form("%s",fileName_.c_str()));
    cout << fileName_ << endl;
    ...
    if(saving_canvas){  // saving canvases
        ...
        compare->SaveAs(Form("%s/efficiency_purity%s%s.pdf",picDirectory.c_str(),absuf.c_str(),suffix.c_str()));
        ...
        canvas_energy->SaveAs(Form("%s/energy_truth_vs_pred%s%s.pdf",picDirectory.c_str(),absuf.c_str(),suffix.c_str()));
        canvas_energy_scan->SaveAs(Form("%s/energy_scan%s%s.pdf",picDirectory.c_str(),absuf.c_str(),suffix.c_str()));
        canvas_energy_resolution_scan->SaveAs(Form("%s/energy_resolution_scan%s%s.pdf",picDirectory.c_str(),absuf.c_str(),suffix.c_str()));
        canvas_beta_energy->SaveAs(Form("%s/beta_vs_energy%s%s.pdf",picDirectory.c_str(),absuf.c_str(),suffix.c_str()));
        canvas_beta_ediff->SaveAs(Form("%s/beta_vs_energy_ediff%s%s.pdf",picDirectory.c_str(),absuf.c_str(),suffix.c_str()));
        canvas_beta_mcen->SaveAs(Form("%s/beta_vs_energy_mcen%s%s.pdf",picDirectory.c_str(),absuf.c_str(),suffix.c_str()));
    }
