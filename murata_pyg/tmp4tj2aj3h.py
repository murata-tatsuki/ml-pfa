...
class Propagate_fac1ab(NamedTuple):
    x: OptPairTensor
    edge_weight: OptTensor



class Collect_fac1ab(NamedTuple):
    x_j: torch.Tensor
...
class GravNetConvJittable_fac1ab(GravNetConv):
    ...
    def __collect__(self, edge_def, size, kwargs):
        # type: (Tensor, List[Optional[int]], Propagate_fac1ab) -> Collect_fac1ab
        pass

    @torch.jit._overload_method
    def __collect__(self, edge_def, size, kwargs):
        # type: (SparseTensor, List[Optional[int]], Propagate_fac1ab) -> Collect_fac1ab
        pass
    ...
    def __collect__(self, edge_def, size, kwargs):
        ...
        return Collect_fac1ab(x_j=x_j, edge_weight=edge_weight, index=index, dim_size=dim_size)
    ...
    def propagate(self, edge_index, x, edge_weight, size=None):
        ...
        in_kwargs = Propagate_fac1ab(x=x, edge_weight=edge_weight)
