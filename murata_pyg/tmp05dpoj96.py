...
class Propagate_c705e1(NamedTuple):
    x: OptPairTensor
    edge_weight: OptTensor



class Collect_c705e1(NamedTuple):
    x_j: torch.Tensor
...
class GravNetConvJittable_c705e1(GravNetConv):
    ...
    def __collect__(self, edge_def, size, kwargs):
        # type: (Tensor, List[Optional[int]], Propagate_c705e1) -> Collect_c705e1
        pass

    @torch.jit._overload_method
    def __collect__(self, edge_def, size, kwargs):
        # type: (SparseTensor, List[Optional[int]], Propagate_c705e1) -> Collect_c705e1
        pass
    ...
    def __collect__(self, edge_def, size, kwargs):
        ...
        return Collect_c705e1(x_j=x_j, edge_weight=edge_weight, index=index, dim_size=dim_size)
    ...
    def propagate(self, edge_index, x, edge_weight, size=None):
        ...
        in_kwargs = Propagate_c705e1(x=x, edge_weight=edge_weight)
