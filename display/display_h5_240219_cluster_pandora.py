...
class Event:
    ...
    # ak_label: hitid, mcid, pdg, charge, mass, px, py, pz (of mcp), status
    # ak_pred: beta, x, y
    # ak_pando: hitid, , clusterid
    @classmethod
    def from_awk_pandora(cls, ak_feat, ak_label, ak_pred, ak_pand, i):
        feat = ak_feat[i]
        label = ak_label[i]
        pred = ak_pred[i]
        pand = ak_pand[i]
        ...
        inst.py = feat[:,8]
        inst.pz = feat[:,9]
        # inst.truth_cluster_idx = pand[:,2]
        inst.truth_cluster_idx = pand
        # inst.truth_cluster_idx = label[:,1]
        inst.pdgid = label[:,2]
        inst.status = ak.values_astype(label[:,8],np.uint32)
        inst.mcpx = label[:,5]
        ...
        for p, x, y, z in zip(pand, inst.x, inst.y, inst.z):
            print(p, x, y, z)
        ...
        for tr in (1, 0):
            ...
            pdata.append(go.Scatter3d(
                ...
                text=[
                    ...
                    for e, t, tr, px, py, pz, s, pdgid
                    in zip(e.energy[sel2], e.time[sel2], e.track[sel2], e.px[sel2], e.py[sel2], e.pz[sel2], e.status_str, e.pdgid[sel2])
                ],
                hovertemplate=(
...
def plot_event_pred(e: Event, use_cluster_color=False):
        ...
        for tr in (1, 0):
            ...
            pdata.append(go.Scatter(
                ...
                text=[
                    ...
                    for beta, x, y, z, energy, track
                    in zip(e.beta[sel2], e.x[sel2], e.y[sel2], e.z[sel2], e.energy[sel2], e.track[sel2])
                ],
                hovertemplate=(
...
# ak_feat, ak_label, ak_pred, _ = tools.load_awkward.load_awkward2(awkfile)
ak_feat, ak_label, ak_pred, ak_energy, ak_pandora = tools.load_awkward.load_awkward2(awkfile)
print(ak_pred)
print(ak_pandora)
# ak_pandora = tools.load_awkward.load_awkward2_pandora(awkfile)
...
for i in range(nstart,nend):
    ...
    # single_pdata_to_file_pred(outhtml, plot_event_pred(e, use_cluster_color=True), include_plotlyjs=True, mode=mode)

    e_pandora = Event.from_awk_pandora(ak_feat, ak_label, ak_pred, ak_pandora, i)
    single_pdata_to_file(outhtml, plot_event(e_pandora), include_plotlyjs=True, mode=mode)
    # mode = 'a'
    # single_pdata_to_file_pred(outhtml, plot_event_pred(e_pandora), include_plotlyjs=True, mode=mode)
    # single_pdata_to_file_pred(outhtml, plot_event_pred(e_pandora, use_cluster_color=True), include_plotlyjs=True, mode=mode)
