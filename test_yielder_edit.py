...
from lcr_module import truth_based_assignment, HungarianMatcher
        ...
        with torch.no_grad():
            ...
            for i, data, out_gravnet in self._iter_data(nmax):
                ...
                if not self.pandora:
                    ...
                    if (not energyRegression and not energyRegressionWeight):
...
class TestYielderGNNTransformer(TestYielder):
    def __init__(self, model_clustering=None, classification=False, pid=False, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.classification = classification
        self.pid = pid
        self.model_clustering = get_model(jit=False) if model_clustering is None else model_clustering
        self.matcher = HungarianMatcher(cls_cost=1.0, bbox_cost=2.0, padding_idx=None)
    ...
    def get_gnn_output(self, batched_data, use_charged_cluster_loss=False, energy_regression=True, energy_regression_cluster=True):
        ...
        return pred_cluster_space_coords.to('cpu'), pred_betas.to('cpu'), pred_tracker_energy.to('cpu'), pred_cluster_energy.to('cpu')
    """

    def get_gnn_output(self, gnn_outputs, batched_data, use_charged_cluster_loss=False, energy_regression=True, energy_regression_cluster=True):
        with torch.no_grad():
            pred_betas = torch.sigmoid(gnn_outputs[:,0])
            pred_charge_track_likeness = None
            pred_tracker_energy = None
    ...
    def _iter_data(self, nmax=None):
        with torch.no_grad():
            ...
            for i, data in enumerate(self.loader):
                ...
                for event_number, event_data, prediction, pred_fourvec, truth_four_vector, mask, attn, fourvec_match in self.iter_event(i, data):
                    event_num = event_number + i * self.batch_size
                    yield event_num, event_data, prediction, pred_fourvec, truth_four_vector, mask, attn, fourvec_match
    ...
    def pdg_id_to_class(self, pdg_ids, pcl_charge):
        """
        pdg_ids: torch.Tensor or np.ndarray, shape (N,)
        Return: torch.Tensor, shape (N,) – class indices (0~9)
        """
    ...
    def iter_event(self, i, data, nmax=None):
        ...
        gnn_outputs, pred_betas = self.get_gnn_output_allFeat(data)
        ...
        if self.pid: 
            pred_fourvec, particle_prob, particle_cls_logits, attn_w, seed_padding_mask = self.model_clustering(hit_embed, hit_mask=hit_mask)

            unique_label = self.split_by_batch(data.label[:,1:4], data.batch)
            unique_label = [torch.unique(t, dim=0) for t in unique_label]
            true_cls = [self.pdg_id_to_class(t[:,1], t[:,2]) for t in unique_label]
        ...
        fourvec_match = self.matcher(particle_cls_logits, pred_fourvec, true_cls, truth_four_vector, seed_padding_mask)

        assert(torch.unique(data.batch).shape[0] == len(pred_fourvec))

        pred_cluster_space_coords, pred_betas, pred_tracker_energy, pred_cluster_energy = self.get_gnn_output(gnn_outputs, data)

        data_list = data.to_data_list()
        for batch_id, data_batch, pred_vecs, truth_vecs, mask, attn in zip(torch.unique(data.batch), data_list, pred_fourvec, truth_four_vector, hit_mask, attn_w):
            same_batch = data.batch==batch_id
            pred_cluster_space_coord = pred_cluster_space_coords[same_batch]
            pred_beta = pred_betas[same_batch]
            tracker_energy = pred_tracker_energy[same_batch]
            cluster_energy = pred_cluster_energy[same_batch]
            # data_batch = Event(Batch.from_data_list([data_batch]))
            data_batch = Batch.from_data_list([data_batch])
            prediction = Prediction(pred_beta, pred_cluster_space_coord, None, None, tracker_energy, cluster_energy, None, None, None, None, None) #w/o noise
            yield i, data_batch, prediction, pred_vecs.to('cpu'), truth_vecs.to('cpu'), mask.to('cpu'), attn.to('cpu'), fourvec_match
