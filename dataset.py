...
def ilc_dataset(root=ReadText("Grav_ILC_setting.txt")["Data Folder"],timingCut=False,test_mode=True):
    print(f"ilc_dataset: {root=}")
    dataset = ILCDataset(root,timingCut=timingCut,test_mode=test_mode)
    #dataset.blacklist()
    return dataset
"""
    ...
    def __init__(self, path, flip=True, reduce_noise: float=None,para_tanh=True, recreate=False, timingCut=False, thetaphi=False, test_mode=False, nstart=0, nend=-1, pandora=False):
        ...
        if(not recreate):
            ...
            if timingCut:
                ...
                self.ak_feats, self.ak_labels = timingCut(self.ak_feats, self.ak_labels, cutoff_time = 14, nstart = nstart, nend = nend)    ## 要変更
