...
class Propagate_03a977(NamedTuple):
    x: OptPairTensor
    edge_weight: OptTensor



class Collect_03a977(NamedTuple):
    x_j: torch.Tensor
...
class GravNetConvJittable_03a977(GravNetConv):
    ...
    def __collect__(self, edge_def, size, kwargs):
        # type: (Tensor, List[Optional[int]], Propagate_03a977) -> Collect_03a977
        pass

    @torch.jit._overload_method
    def __collect__(self, edge_def, size, kwargs):
        # type: (SparseTensor, List[Optional[int]], Propagate_03a977) -> Collect_03a977
        pass
    ...
    def __collect__(self, edge_def, size, kwargs):
        ...
        return Collect_03a977(x_j=x_j, edge_weight=edge_weight, index=index, dim_size=dim_size)
    ...
    def propagate(self, edge_index, x, edge_weight, size=None):
        ...
        in_kwargs = Propagate_03a977(x=x, edge_weight=edge_weight)
