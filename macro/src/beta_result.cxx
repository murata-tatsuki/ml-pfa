...
const bool beta_scan = true; 

const double Pandora_eff[3] = {0.993, 0.940, 0.991};
const double Pandora_pur[3] = {0.918, 0.946, 0.972};
...
void beta_result(){ 
    ...
    if(hyper_parameter && fine_tuning && beta_scan){ // condition check
    ...
    // filein[0] = new TFile("../output/uds_to_uds/gravnet/tc_usd91_timingcut_forcealpha_thetaphi_16_usd.root");
    ...
    if(beta_scan){
        for(int ifile=0;ifile<rawfilenum;ifile++){
            filein[ifile] = new TFile(Form("../output/hyper_parameter/beta_threshold/tc_ntau_10GeV_10_timingcut_forcealpha_thetaphi_5D_49_ntau_10GeV_10/beta%03d.root",ifile));
        }
    }
    ...
    TGraph *eff_vs_pur_[10][nParticle];
    for(int thre=0;thre<10;thre++){
        for(int ip=0;ip<nParticle;ip++){
            eff_vs_pur_[thre][ip] = new TGraph();
            eff_vs_pur_[thre][ip]->SetTitle(Form("%s",particleNames[ip].c_str()));
            eff_vs_pur_[thre][ip]->GetXaxis()->SetTitle("efficiency");
            eff_vs_pur_[thre][ip]->GetYaxis()->SetTitle("puriy");
            eff_vs_pur_[thre][ip]->SetMarkerStyle(4);
            eff_vs_pur_[thre][ip]->SetMarkerSize(0.5);
            eff_vs_pur_[thre][ip]->SetMarkerColor(thre+1);
        }
    }
    ...
    for(int irawfile=0; irawfile<rawfilenum; irawfile++){
        ...
        for(int ip=0;ip<nParticle;ip++){
            ...
            if(irawfile%10==0) eff_vs_pur_[irawfile%10][ip]->SetPoint(irawfile, efficiency_scan[ip][irawfile], purity_scan[ip][irawfile]);
        }
        ...
        if(beta_scan) delete filein[irawfile];
    }
    ...
    for(int ip=0;ip<nParticle;ip++){
        ...
        Pandora_eff_vs_pur[ip]->SetMarkerStyle(4);
    ...
    for(int ip=0;ip<nParticle;ip++){
        ...
        compare->cd(ip+1);
        // gPad->SetLogy();
        if(ip==1) eff_vs_pur[ip]->SetMinimum(0.935);
        eff_vs_pur[ip]->Draw();
        for(int thre=0;thre<10;thre++) {
            // cout << "ip:" << ip << " thre:" << thre << endl;
        }
