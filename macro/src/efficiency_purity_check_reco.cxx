        ...
        for (int j = i; j <= nBins; ++j) {
            ...
            if (sum >= targetEntries) {
                ...
                if (width < minWidth) {
                    ...
                    double mean = sumX / nSub;
                    bestRMS90 = TMath::Sqrt(sumX2 / nSub - mean * mean);
                }
...
void efficiency_purity_check_reco(){ 
    ...
    if(rawfilenum == 1) fileNames_raw[0] = Form("%s",fileName.c_str());
        ...
        for(int qq=0; qq<3; qq++){
            for(int i=1; i<=nfile; i++){
                ...
                fileNames[qq][i] = Form("../output/energy_regression_1to1/skimmed/tc_fixed_uds/5D/E_regression/tbeta_td_scan/qmin02_lr5e-4/%dGeV/truth_clustering/%s_%03d.root", qq_energy, qqNames[qq].c_str(), i);
    ...
    for(int irawfile=0; irawfile<rawfilenum; irawfile++){
        ...
        if(qq_energy==500 && (irawfile==36 || (irawfile>=48 && irawfile<56) || irawfile==171 || irawfile==247)) continue;
