...
class TestYielderWithMLClustering(TestYielder):
    ...
    def _iter_data(self, nmax=None):
        with torch.no_grad():
            ...
            for i, data in enumerate(self.loader):
                ...
                for event_number, event_data, pred_fourvec, truth_four_vector in self.iter_event(i, data):
                    event_num = event_number + i * self.batch_size
                    yield event_num, event_data, pred_fourvec, truth_four_vector
    ...
    def iter_event(self, i, data, nmax=None):
        ...
        data_list = data.to_data_list()
        for batch_id, data_batch, pred_vecs, truth_vecs in zip(torch.unique(data.batch), data_list, pred_fourvec, truth_four_vector):
            data_batch = Batch.from_data_list([data_batch])
            yield i, data_batch.to('cpu'), pred_vecs.to('cpu'), truth_vecs.to('cpu')
