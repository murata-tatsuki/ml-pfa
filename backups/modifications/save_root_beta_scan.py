...
def save_root(datapath, ckpt, outfileDir, nstart=0, nend=-1, timingCut=False, input_dim=5, output_dim=3, pandora=False):
    ...
    for tbeta_now in range(100):
        tbeta = tbeta_now / 100
        outfile = outfileDir + '/beta' + format(tbeta_now, '03') + '.root'
        ...
        for i, (event, prediction, clustering, matches) in enumerate(yielder.iter_matches(tbeta=tbeta, td=0.5, nmax=nmax, pandora=pandora)):
