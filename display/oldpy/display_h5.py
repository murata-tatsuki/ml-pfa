...
class Event:
    ...
    @classmethod
    def from_awk(cls, ak_feat, ak_label, i):
        feat = ak_feat[i]
        label = ak_label[i]
        inst = cls()
        inst.x = feat[:,1]
...
def single_pdata_to_file(
    outfile, pdata, mode='w', title=None, width=1200, height=None, include_plotlyjs='cdn'
    ):
    import plotly.graph_objects as go
...
ak_feat, ak_label = tools.load_awkward.load_awkward2(awkfile)
...
for i in range(nstart,nend):
    e = Event.from_awk(ak_feat, ak_label, i)
